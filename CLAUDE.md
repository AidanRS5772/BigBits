...
- **`div.rs`** — Division exposes six dispatched operations: quotient-only, quotient-with-remainder, and reciprocal, each with dynamic and static allocation models. Quotient operations dispatch among Knuth, Burnikel-Ziegler, and Newton-Raphson; reciprocal operations dispatch directly between Knuth and Newton-Raphson and have no standalone Burnikel-Ziegler path. Forced algorithm entry harnesses live in `src/tests/test_div.rs` and `benches/`, where they compose the public preflight and wrapper stages; they do not live in the algorithm module.

  Division operands must be trimmed and `d` must be nonzero. Let `L = div_quotient_len(n.len(), d.len())`, including its possible zero top structural limb. A quotient buffer of any length is valid: the division entries compute the high `min(q.len(), L)` limbs of `floor(n / d)`, pack them at the low end of `q`, and zero-pad `q[L..]` when the buffer is oversized. Thus a short `q` requests a high-quotient truncation rather than causing a shape error. Quotient-only entries preserve both operands.

  Each call follows three explicit layers. The public API preflight validates the trimmed operands, zeroes the output, handles empty work or `n < d`, computes the requested quotient width, and selects one algorithm. The selected algorithm wrapper then drops the low numerator limbs that cannot affect the requested high quotient, removes any exact whole-limb `B` factor represented by low zero limbs of `d` together with the corresponding numerator limbs, checks static capacity after trimming, and prepares mutable scratch buffers. Finally, the rigid Knuth, Burnikel-Ziegler, or Newton-Raphson core executes with the exact full shape `n.len() = d.len() + q.len() - 1`. Tests and benchmarks force an algorithm by locally calling the same preflight followed by the desired wrapper.

  `*_div_rem_*` entries preserve `d` and write a residual into `n`. For a truncated request, let `k = min(q.len(), L)`, `s = L - k`, and interpret the returned quotient and mutated numerator as integers `Q` and `R_partial`. They satisfy `N_original = Q * D * B^s + R_partial` and `0 <= R_partial < D * B^s`. Consequently the residual is an exact remainder for the shifted high-quotient problem, but it is `N_original % D` only when the full structural quotient is requested (`q.len() >= L`; the `n < d` zero-work case is full trivially).
