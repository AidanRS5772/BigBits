use crate::utils::{
    div::{div_dyn, div_rem_dyn, div_rem_static, div_static, knuth_est, mul_u64_asm},
...
fn zimmermann_sqrt_core(
    ...
    if saturated {
        ...
        // Trim within the numerator window: it may be empty even when the
        // radicand itself is nonzero (for example, a power-of-two square).
    ...
    // s_lo^2 < B^(2*lo). A nonzero limb above that boundary proves that
    // subtraction cannot make the remainder negative, without a square.
    if output == SqrtOutput::Root && x[2 * lo..s_len + 1].iter().any(|&limb| limb != 0) {
        return;
    }
...
fn zimmerman_sqrt_top(
    ...
    // The low lo limbs are virtual zeros: Rpre = value(x) * B^lo.
    if output == SqrtOutput::Root && x[lo..].iter().any(|&limb| limb != 0) {
        return None;
    }

    x.copy_within(..x_len - lo, lo);
    x[..lo].fill(0);
    // Let the allocation-specific wrapper obtain square scratch only when
    // correction or an exact remainder actually requires the square.
    Some(2 * lo)
}
...
pub fn zimmerman_sqrt_dyn(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_dyn_output(x, s, SqrtOutput::RootRem);
}
...
pub fn zimmerman_sqrt_only_dyn(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_dyn_output(x, s, SqrtOutput::Root);
}
...
pub fn zimmerman_sqrt_approx_dyn(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_dyn_output(x, s, SqrtOutput::ApproxRoot);
}

fn zimmerman_sqrt_dyn_output(x: &mut [u64], s: &mut [u64], output: SqrtOutput) {
    // assume correct size bounds
    debug_assert!(x.len() > s.len());
    debug_assert!(2 * s.len() >= x.len());
...
pub fn zimmerman_sqrt_static<const N: usize>(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_static_output::<N>(x, s, SqrtOutput::RootRem);
}
...
pub fn zimmerman_sqrt_only_static<const N: usize>(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_static_output::<N>(x, s, SqrtOutput::Root);
}
...
pub fn zimmerman_sqrt_approx_static<const N: usize>(x: &mut [u64], s: &mut [u64]) {
    zimmerman_sqrt_static_output::<N>(x, s, SqrtOutput::ApproxRoot);
}

fn zimmerman_sqrt_static_output<const N: usize>(x: &mut [u64], s: &mut [u64], output: SqrtOutput) {
    // assume correct size bounds
    debug_assert!(x.len() > s.len());
    debug_assert!(2 * s.len() >= x.len());
    debug_assert!(
