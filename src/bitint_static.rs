use crate::ubitint_static::{ToUsizeArray, UBitIntStatic, add_ubis, one_comp};
use std::arch::asm;
use std::ops::*;
...
impl<const N:usize> BitIntStatic<N>{
    ...
    #[inline]
    pub fn from<T: ToUsizeArray<N> + std::cmp::PartialOrd + Default + std::marker::Copy>(val: T) -> BitIntStatic<N>{
        BitIntStatic::<N>{ data: val.to_usize_array(), sign: val < T::default()}
    }
