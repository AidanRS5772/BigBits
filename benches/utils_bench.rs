...
use big_bits::utils::sqrt::{
    binom_sqrt, zimmerman_sqrt_approx_dyn, zimmerman_sqrt_approx_static, zimmerman_sqrt_dyn,
...
fn register_sqrt_only_size<const N: usize>(
    ...
    let entries: [(&str, fn(&mut [u64], &mut [u64])); 6] = [
        ("rem_dyn", zimmerman_sqrt_dyn),
        ("only_dyn", zimmerman_sqrt_only_dyn),
        ("approx_dyn", zimmerman_sqrt_approx_dyn),
        ("rem_static", zimmerman_sqrt_static::<N>),
        ("only_static", zimmerman_sqrt_only_static::<N>),
        ("approx_static", zimmerman_sqrt_approx_static::<N>),
    ];
